...
# Coolify Trading Signal Bot v026
...
# v026 local commodity candles come directly from MEXC Futures. DXY/US10Y are
...
def _technical_from_rows(rows: list[list[Any]], interval: str) -> dict[str, Any]:
    # v026: keep younger listings usable instead of requiring a full 365D
...
async def _fetch_binance_spot_details(
    ...
    v026 requests up to 365 closed 1D candles and 288 closed 15m candles.
...
async def _fetch_mexc_derivatives(
    ...
    In v026 MEXC is the only funding source; Binance Futures is not queried.
    """
    try:
        payload = await _get_json(
            client,
...
async def _fetch_derivatives(
    ...
    *,
    ...
    Funding is intentionally MEXC-only in v026. Binance Futures is not called at
...
async def _fetch_mexc_commodity_asset(
    ...
    No alternate chart-provider fallback is used in v026. 1D is retained up to 365 bars, native 4H
...
async def _fetch_calendar(client: httpx.AsyncClient) -> tuple[list[dict[str, Any]], str]:
    ...
    # The old ff_calendar_nextweek.json URL returns 404. v026 instead parses
...
async def fetch_market_bundle() -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (compatible; CoolifyTradingSignalBot/v026)",
    }
...
async def fetch_signal_histories(records: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Fetch lightweight 1H history for v026 signal statistics.
    ...
    async with httpx.AsyncClient(timeout=timeout, follow_redirects=True, headers=headers) as client:
        async def fetch_one(record: dict[str, Any]) -> tuple[str, list[dict[str, Any]] | None]:
            ...
            if record.get("market") == "commodity":
                # Migrate pending legacy commodity records transparently: v026 always

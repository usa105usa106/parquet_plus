...
async def _fetch_mexc_derivatives(
    ...
    In v005 MEXC is the only funding source; Binance Futures is not queried.
    """
    try:
...
async def _fetch_derivatives(
    ...
    Funding is intentionally MEXC-only in v005. Binance Futures is not called at
...
async def fetch_market_bundle() -> dict[str, Any]:
    headers = {
        ...
        "User-Agent": "Mozilla/5.0 (compatible; CoolifyTradingSignalBot/v005)",
    }
...
async def fetch_signal_histories(records: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
    """Fetch lightweight 1H history for v005 signal statistics.
